...
class TestOnMessage:
    ...
    async def test_handler_never_raises_when_db_unavailable(
        self, monkeypatch, translator
    ) -> None:
        """
        Регрессионный тест на найденную проблему: ChatRepository.upsert и
        _build_context вызывались ДО try/except внутри on_message — если БД
        временно недоступна (обрыв соединения, таблицы не созданы), это
        роняло обработку КАЖДОГО обычного сообщения в чате (не только само
        нарушение, а вообще любое сообщение). Теперь весь хэндлер обёрнут
        в try/except целиком.
        """
        from contextlib import asynccontextmanager

        @asynccontextmanager
        async def broken_session_scope():
            raise RuntimeError('relation "chats" does not exist')
            yield  # pragma: no cover

        monkeypatch.setattr("bot.session_scope", broken_session_scope)

        bot = AsyncMock()
        message = _message(100, 5, "любое сообщение")

        # Не должно бросить исключение наружу, несмотря на полностью
        # недоступную БД на самом первом обращении внутри хэндлера.
        await router_module.on_message(message, bot, translator, "ru")
